...
* Match the CLI version to the pin shipped with the official instructions. Verify the official checksum, host and architecture before installing. Do not silently replace an existing mismatched engine or guess a compatible version. Madison's optional CapCut adapter and editing mode accept Tesseract 0.2.0 or 0.1.0 and refuse other versions.
* Madison's version label shows an amber dot when Mirage ships a Tesseract version this Madison does not support yet, or when the installed engine is unsupported. Tell the user, and update Madison or the engine before editing.
* On Windows, if Tesseract preview or export exits with code 0xC000001D, another application's `dxcompiler.dll` on PATH is shadowing the renderer. Obsidian is a known cause. Madison removes the Obsidian folder from PATH for its own Tesseract calls; when an agent runs Tesseract directly, remove that folder from PATH for that command only.
...
Keep generated reviews outside this repository and preserve source projects and media. Check picture and audio timing. Tesseract 0.1.0 has shown native audio source offset differences, not yet ruled out in 0.2.0; a completed render does not prove an accurate soundtrack. Say so if audio cannot be auditioned.
