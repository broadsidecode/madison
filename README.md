...
A free local review screen for people working with video editors and AI agents.
...
![Timeline Reviewer with the synthetic community demo](images/viewer-demo.png)

## Start with the demo

Using Codex? [Open the setup guide and copy its prompt](START-HERE.md).
...
[Full controls guide](docs/viewer-controls.md)
...
That creates one review clip covering the whole movie. To show individual lanes and cuts, supply a manifest or have your agent produce one using the [manifest format](docs/manifest.md):
...
[Importer setup and supported behavior](docs/experimental-importer.md)
...
Importer unit tests use synthetic fixtures and a mocked editor. The FFmpeg preparation test skips when those optional tools are absent. A symlink test may skip where the operating system does not permit creating symlinks. See [verification notes](docs/verification.md) for what has and has not been tested.
